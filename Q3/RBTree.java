...
public class RBTree<T extends Comparable<T>> {
	...
	public boolean checkEnd(Node node){
		...
		return checkEnd(node.left) && checkEnd(node.right);
	}

	/**
	 * Checks if a red node has a black parent.
	 * @return true if property holds, false otherwise.
	 */
	public boolean testProp2() {
		// START YOUR CODE

		return checkRB(root); //you are allowed to change this return statement
		// END YOUR CODE
	}

	public boolean checkRB(Node node){
		if(node == null){
			return true;
		}
		if(node.colour == Colour.RED){
			if(node.parent != null && node.parent.colour != Colour.BLACK){
				return false;
			}
		}
