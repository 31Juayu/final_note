...
public class RBTree<T extends Comparable<T>> {
	...
	public boolean testProp3() {
		// START YOUR CODE
		List<List<Node<T>>> list = findAllPaths();
	...
	private void collectPaths(Node<T> node, List<Node<T>> currentPath, List<List<Node<T>>> paths) {
		if (node == null || node.value == null) { // Base case: reach a leaf node
			return;
		}
		...
		// Check if it's a leaf node (both children are NIL nodes)
		if (node.left.value == null && node.right.value == null) {
			// If it's a leaf node, add the current path to the paths list
			paths.add(new ArrayList<>(currentPath));
		} else {
			// Recursively collect paths from the left and right children
